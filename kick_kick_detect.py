...
class kick_kick_detect:
  def __init__(self, confthres=0.3, nmsthres=0.1):
    self.yolo_path = './'
...
Lables=detectionObj.get_labels(labelsPath)
CFG=detectionObj.get_config(cfgpath)
Weights=detectionObj.get_weights(wpath)
nets=detectionObj.load_model(CFG,Weights)
Colors=detectionObj.get_colors(Lables)
...
predicted_image = detectionObj.get_predection(image, nets, Lables, Colors)

image = cv2.cvtColor(predicted_image, cv2.COLOR_BGR2RGB)
np_img = Image.fromarray(image)
np_img.save('./test_image_predicted.jpg')